...
/// garden walls stay clean porcelain panels (owner directive 2026-07-12).
fn wall_runs(g: &Grid) -> Vec<WallRun> {
    let (w, h) = (g.w, g.h);
    let mut out = Vec::new();
    let roomy = |p: CellPos| g.in_bounds(p) && g.cell(p) == CellKind::Room;
    // Z-boundaries (edges between (x,z-1) and (x,z), drawn per cell's Zm):
    for z in 0..h {
        let zroomy = |x: i16| roomy(CellPos::new(x, z)) || (z > 0 && roomy(CellPos::new(x, z - 1)));
        let mut x = 0i16;
        while x < w {
            if g.edge(CellPos::new(x, z), Dir::Zm) == EdgeKind::Wall {
                ...
                out.push(WallRun { rect: [x0 as f32 - WALL_HT, z as f32 - WALL_HT, x as f32 + WALL_HT, z as f32 + WALL_HT], along_x: true, windows: rm });
            } else {
                x += 1;
            }
        }
    ...
    for x in 0..w {
        let xroomy = |z: i16| roomy(CellPos::new(x, z)) || (x > 0 && roomy(CellPos::new(x - 1, z)));
        let mut z = 0i16;
        while z < h {
            if g.edge(CellPos::new(x, z), Dir::Xm) == EdgeKind::Wall {
                ...
                out.push(WallRun { rect: [x as f32 - WALL_HT, z0 as f32 - WALL_HT, x as f32 + WALL_HT, z as f32 + WALL_HT], along_x: false, windows: rm });
            } else {
                z += 1;
            }
        }
...
pub fn build_gym(spec: &GymLevel, look: &Look, roof: bool) -> (Scene, GymMeta) {
    ...
    if roof && spec.neighborhood {crate::terrain::roofs(&mut scene);}
    if roof && !spec.neighborhood {
...
mod tests {
    ...
    fn the_greybox_is_boxes_and_every_pier_mesh_is_its_authored_box() {
        ...
        for pier in &meta.piers {
            ...
            assert!((lo - pier.lo).length() < 1e-5 && (hi - pier.hi).length() < 1e-5, "pier box {lo:?}..{hi:?} != authored {:?}..{:?}", pier.lo, pier.hi);
        }
    }
