...
function makeCore(
  overrides: Partial<ConstructorParameters<typeof IsoViewport>[0]> = {}
): IsoViewport {
  return new IsoViewport({
    width: 160,
    height: 120,
...
function makeSideCore(): IsoViewport {
...
describe("@common/render iso viewport core", () => {
  ...
  it("respects configured zoomMax when stepping zoom (regression for hardcoded 20)", () => {
    // Before the fix the controller hard-coded maxZoom: 20, silently
    // overriding the tuning's zoomMax. With zoomMax=4 and basePixelZoom=2,
    // issuing zoom-in steps must cap at 4 (not 20).
    const core = makeCore({ basePixelZoom: 2, zoomMax: 4, zoomStep: 1 });
    for (let i = 0; i < 20; i += 1) {
      core.zoomStepAtLocalCss(1, 80, 60, 1000 + i * 16);
    }
    expect(core.getState().cameraZoomTarget).toBe(4);
  });
