...
export type SharedScissorStageConfig = {
  ...
  clearAlpha?: number;
  /**
   * When true, the shared renderer is configured with
   * `shadowMap.enabled = true` and `shadowMap.type = PCFSoftShadowMap`.
   * Shadow casting is a per-renderer flag, so it lives here rather than
   * on individual panes.
   */
  shadows?: boolean;
};
