...
function ForgeShellInner({ renderDrawer }: ForgeShellProps) {
  ...
  return (
    <div className="ps-shell forge-app" data-testid="forge-page">
      {renderDrawer?.(menuOpen, () => setMenuOpen(false))}
...
export function ForgeShell({ renderDrawer }: ForgeShellProps) {
  return (
    <ForgeProvider>
      <ForgeShellInner renderDrawer={renderDrawer} />
    </ForgeProvider>
  );
}
