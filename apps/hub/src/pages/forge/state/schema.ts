...
function normalizeBBox(value: unknown): ForgeBBoxJson | undefined {
...
export function defaultForgeColliderPresetFile(): ForgeColliderPresetFile {
  const presets = [
    {
      id: "fast-preview",
      name: "Fast Preview",
...
export function sanitizeForgeColliderPresetFile(raw: unknown): ForgeColliderPresetFile {
  const record = asRecord(raw);
  const sourcePresets = Array.isArray(record?.presets) ? record.presets : [];
  const seen = new Set<string>();
  const presets: ForgeColliderPresetFile["presets"] = [];
  ...
  if (presets.length <= 0) {
    return defaultForgeColliderPresetFile();
  }
...
export function defaultForgePhysicsKindPresetFile(): ForgePhysicsKindPresetFile {
  return {
    version: 1,
    defaultKindId: "wood",
    updatedAt: new Date().toISOString(),
...
export function sanitizeForgePhysicsKindPresetFile(raw: unknown): ForgePhysicsKindPresetFile {
  const record = asRecord(raw);
  const sourceKinds = Array.isArray(record?.kinds) ? record.kinds : [];
  const kinds: ForgePhysicsKindPresetFile["kinds"] = [];
  const seen = new Set<string>();
  ...
  if (kinds.length <= 0) {
    return defaultForgePhysicsKindPresetFile();
  }
...
export function createDefaultForgeMeta(input: {
  id: string;
  description: string;
...
export function sanitizeForgePropMeta(raw: unknown): ForgePropMeta | null {
  const record = asRecord(raw);
  if (!record) {
    return null;
  }
    ...
    colliders: collidersRecord
      ? {
          ...
          presets: Array.isArray(collidersRecord.presets)
            ? (collidersRecord.presets as ForgeColliderResultEntry[])
            : []
        }
      : undefined,
    physics: physicsRecord
      ? {
          ...
          simulationChecks: (asRecord(physicsRecord.simulationChecks) ?? undefined) as
            | Record<string, ForgeSimulationCheckResult>
            | undefined
        }
      : undefined
  };
