...
impl Canvas {
    ...
    fn fill(&mut self, x0: i32, y0: i32, x1: i32, y1: i32, c: [u8; 3]) {
        for y in y0..y1 {
            for x in x0..x1 {
                self.put(x, y, c);
            }
    ...
    fn disc(&mut self, cx: i32, cy: i32, r: i32, c: [u8; 3]) {
        for y in -r..=r {
            for x in -r..=r {
                if x * x + y * y <= r * r {
    ...
    /// Draw one 3x5 glyph at scale. Returns advance width in px.
    ...
    fn text(&mut self, s: &str, x: i32, y: i32, scale: i32, c: [u8; 3]) -> i32 {
        let mut cx = x;
        for ch in s.chars() {
...
/// Compact 3x5 bitmap font: digits, the eight letters used in the header
/// (D E L M O P R S), space and '/'. Each row's low 3 bits are the pixels.
